...
async def _start_round(cid, ctx):
    ...
    await asyncio.sleep(1)  # small delay for webhook
...
async def cmd_reset(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cid in games:
        games.pop(cid)
        await update.message.reply_text(
            "✅ <b>Game reset ho gayi!</b>\n\nAb /startgame se naya game shuru karo! 🎮",
            parse_mode="HTML",
        )
    else:
        await update.message.reply_text("ℹ️ Koi active game nahi thi.\n/startgame se naya game shuru karo!")
